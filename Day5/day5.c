...
int main(int argc, char** argv) {
    ...
    while((line = fgets_dup(f)) && strlen(line) > 0) {
        ...
        if(!line_safe) {
            for(int findpos = lnums.len; findpos > 0; findpos--) {
                bool found = false;
                for(int trypos = 0; trypos < findpos; trypos++) {
                    int tryval = lnums.contents[trypos];
                    for(int accpos = 0; accpos < findpos; accpos++) {
                        int accval = lnums.contents[accpos];
                        if(accpos != trypos && accval < ents_len) {
                            entry_merge(&ent, &ents[accval]);
                        }
                    }
                    if(!entry_has(&ent, tryval)) {
                        lnums.contents[trypos] = lnums.contents[findpos-1];
                        lnums.contents[findpos-1] = tryval;
                        found = true;
                        entry_destroy(&ent);
                        break;
                    }
